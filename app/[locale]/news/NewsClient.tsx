...
function ArticleWrapper({ article, locale, children }: { article: NewsArticle; locale: string; children: React.ReactNode }) {
  ...
  if (hasBody) return <Link href={`/${locale}/news/${article.slug}`} className="block">{children}</Link>
  if (article.sourceUrl) return <a href={article.sourceUrl} target="_blank" rel="noopener noreferrer" className="block">{children}</a>
  return <>{children}</>
}
                    ...
                    <div className="flex flex-col flex-1 p-6">
                      <div className="flex items-center gap-3 mb-3">
                        <time dateTime={article.date} className="font-ui text-label uppercase tracking-[0.12em] text-ash">
                          {new Date(article.date).toLocaleDateString(locale === 'vi' ? 'vi-VN' : 'en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </time>
                        <span className="text-bark">·</span>
                        <span className="font-ui text-label uppercase tracking-[0.12em] text-ash">{article.readingTime} {t('page.news.minRead')}</span>
                      </div>
                      <h2 className="font-display text-xl text-parchment leading-snug mb-3 flex-1">{article.title}</h2>
                      <p className="font-body text-ash text-sm leading-relaxed mb-4 line-clamp-3">{article.excerpt}</p>
                      <div className="flex items-center justify-between mt-auto pt-4 border-t border-bark">
                        <span className="font-ui text-label uppercase tracking-[0.12em] text-ash">{t('page.news.by')} {article.author}</span>
                        <span className="font-ui text-label uppercase tracking-[0.12em] text-saffron group-hover:text-parchment transition-colors">{t('page.news.readMore')} →</span>
                      </div>
                    </div>
                  </div>
