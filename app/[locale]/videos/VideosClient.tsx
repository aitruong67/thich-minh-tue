...
export default function VideosClient({ initialVideos, total, pageSize }: Props) {
  const t       = useTranslations()
  const locale  = useLocale()
  const [active, setActive]     = useState<Category>('all')
  const [videos, setVideos]     = useState<Video[]>(initialVideos)
  const [loading, setLoading]   = useState(false)
  const [hasMore, setHasMore]   = useState(videos.filter(v => !v.pinned).length >= pageSize)
  // offset tracks how many non-pinned videos have been loaded
  const [offset, setOffset]     = useState(initialVideos.filter(v => !v.pinned).length)

  const loadMore = useCallback(async () => {
    setLoading(true)
    try {
      const res  = await fetch(`/api/videos?offset=${offset}&limit=${pageSize}`)
      const data = await res.json()
      setVideos(prev => [...prev, ...data.videos])
      setHasMore(data.hasMore)
      setOffset(o => o + data.videos.length)
    } finally {
      setLoading(false)
    }
  }, [offset, pageSize])
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {filtered.map((video, i) => (
              <VideoCard key={video._id} video={video} locale={locale} t={t} index={i} />
            ))}
          </div>
          ...
          {hasMore && active === 'all' && (
            <div className="flex flex-col items-center gap-2 mt-14">
              <button
                onClick={loadMore}
                disabled={loading}
                className="btn-ghost disabled:opacity-50 disabled:cursor-not-allowed min-w-[200px]"
              >
                {loading
                  ? (locale === 'vi' ? 'Đang tải…' : 'Loading…')
                  : (locale === 'vi'
                    ? `Xem thêm (${total - videos.length} video)`
                    : `Load more (${total - videos.length} videos)`)}
              </button>
              <p className="font-ui text-[0.6rem] uppercase tracking-widest text-ash/40">
                {locale === 'vi'
                  ? `Đang hiển thị ${videos.length} / ${total} video`
...
function VideoCard({ video, locale, t, index }: {
  video: Video; locale: string; t: ReturnType<typeof useTranslations>; index: number
}) {
  return (
    <FadeIn key={video._id} delay={Math.min(index, 8) * 0.05} as="article">
      <Link href={`/${locale}/videos/${video.slug}`}
        className="flex flex-col h-full border border-bark hover:border-saffron/40 transition-colors group">
        {/* Thumbnail */}
        <div className="relative aspect-video overflow-hidden bg-bark/30">
          {video.thumbnailUrl ? (
            <Image src={video.thumbnailUrl} alt={locale === 'vi' ? video.title_vi : video.title_en}
            ...
            <video src={video.videoUrl} className="absolute inset-0 w-full h-full object-cover"
              preload="metadata" muted playsInline />
          ) : (
            <div className="absolute inset-0 bg-bark/20 flex items-center justify-center">
              <svg className="w-10 h-10 text-bark" fill="currentColor" viewBox="0 0 24 24">
